...
export const REQUEST_PATIENT_HISTORY_LINES_SUCCESS = "REQUEST_PATIENT_HISTORY_LINES_SUCCESS";
...
export const getPatientHistorySlice = (treatmentNo) => async (dispatch, getState) => {
  try {
    ...
    dispatch({ type: REQUEST_PATIENT_HISTORY_LINES_SUCCESS, payload: response.data });
  } catch (error) {
    const errorMessage =
    ...
    dispatch({ type: REQUEST_PATIENT_HISTORY_LINES_FAIL, payload: errorMessage });
  }
};
