...
export const saveGeneralInformation =
  (formData, navigate) => async (dispatch, getState) => {
    try {
      ...
      const patientNo =
        response.data.patientNo || response.data.patient?.patientNo || null;
      ...
      dispatch({
        type: SAVE_GENERAL_INFORMATION_SUCCESS,
        payload: { ...response.data, patientNo },
      });
