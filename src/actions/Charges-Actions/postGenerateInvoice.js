...
export const GENERATE_SHA_INVOICE_REQUEST = "GENERATE_SHA_INVOICE_REQUEST";
export const GENERATE_SHA_INVOICE_SUCCESS = "GENERATE_SHA_INVOICE_SUCCESS";
export const GENERATE_SHA_INVOICE_FAIL = "GENERATE_SHA_INVOICE_FAIL";
export const GENERATE_SHA_INVOICE_RESET = "GENERATE_SHA_INVOICE_RESET";
...
export const generateSHAInvoice = (payload) => async (dispatch, getState) => {
  try {
    dispatch({ type: GENERATE_SHA_INVOICE_REQUEST });
    ...
    // Make the POST request to the server
    const response = await axios.post(
      `${API}Reception/GenerateRebatesInvoice`,
      payload,
      config
    );

    console.log("Generate SHA Invoice response:", response.data);

    // Extract and validate the response data
    // const { status } = response.data;
    dispatch({
      type: GENERATE_SHA_INVOICE_SUCCESS,
    ...
    // Dispatch failure action
    dispatch({
      type: GENERATE_SHA_INVOICE_FAIL,
      payload: errorMessage,
    });

    console.log("Error generating SHA Invoice:", errorMessage); 
    // Display error message
    message.error(errorMessage);
