...
export const getQyInpatientProcessProceduresSlice = () => 
  async (dispatch, getState) => {
    const config = configHelpers(getState);
    try {
        dispatch({ type: GET_QY_INPATIENT_PROCESS_PROCEDURE_REQUEST });

        const { data } = await axios.get(`${API_URL}/data/odatafilter?webservice=QyInpatientProcessProcedures&isList=true`, config);
