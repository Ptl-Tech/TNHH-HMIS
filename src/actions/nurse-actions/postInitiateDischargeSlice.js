...
export const postDischargeSummary = (dischargeSummary) =>
    async (dispatch, getState) => {
        const config = configHelpers(getState);
        try {
            dispatch({ type: POST_DISCHARGE_SUMMARY_REQUEST });

            const { data } = await axios.post(`${API_URL}/Inpatient/DischargeSummary`, dischargeSummary, config);
...
export const getDischargeSummary = (admissionNo) =>
    async (dispatch, getState) => {
        const config = configHelpers(getState);
        try {
            dispatch({ type: GET_DISCHARGE_SUMMARY_REQUEST });

            const { data } = await axios.get(`${API_URL}/data/odatafilter?webservice=QyDischargeSummaryList&isList=true&query=$filter=Discharge_Code eq '${admissionNo}'`, config);
