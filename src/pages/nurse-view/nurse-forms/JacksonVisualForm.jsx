...
const JacksonVisualForm = () => {
  ...
  const canCreateJacksonVisual = ability.can("create", "jacksonVisual");
  ...
  const { loadingGetJacksonVisual, getJacksonVisual } = useSelector(
    (state) => state.getJacksonVisualForm
  );
  const { loadingJackson } = useSelector(
    (state) => state.postJacksonVisualForm
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
        {!isFormVisible &&
          (canCreateJacksonVisual ? (
            <>
              <Button type="primary" onClick={handleButtonVisibility}>
                <PlusOutlined /> New Jackson Form
              </Button>
              <Button
                type="primary"
