...
const Admissions = () => {
  ...
  const { loadingConsultationRoomList, consultationRoomList } = useSelector(
    (state) => state.getConsultationRoom
  );
  ...
  const handleNavigate = (record) => {
    if (user.departmentName === "Nurse") {
      navigate(
        `/Dashboard/Consultation/Patient?TreatmentNo=${record?.TreatmentNo}`,
        {
  ...
  const combinedListWithDoctors = useMemo(() => {
    return combinedList?.map((item) => {
      const matchingDoctor = formattedDoctorDetails?.find(
        (doctor) => doctor?.DoctorID === item?.DoctorID
      );
      return {
        ...item,
  ...
  return (
    <div style={{ margin: "20px 10px 10px 10px" }}>
      ...
      <FilterConsultationRoom
        ...
        setSearchAdmissionNumber={setSearchVisitNumber}
      />
