  ...
  const handleOnFinish = async (values) => {
    try {
      ...
      const dispatchVisitorData = async (data) => {
        await dispatch(
          postVisitorListSlice("/InpatientForms/VisitorsListForm", data)
        )
          .then((result) => {
            if (result.type === POST_VISITOR_LIST_SUCCESS) {
              message.success(
                result.payload.msg || `Visitor has been saved successfully!`
              );
                ...
                rules={[
                  { required: true, message: "Phone Number is required" },
                  {
                    validator: (_, value) => {
                      ...
                      return Promise.reject(
                        "PPlease enter a valid phone number"
                      );
                    },
                  },
                ]}
