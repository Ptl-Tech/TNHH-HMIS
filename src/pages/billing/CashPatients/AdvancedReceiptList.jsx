...
import { getReceipts } from "../../../actions/Charges-Actions/getReceipts";
 ...
 const [filteredData, setFilteredData] = useState([]);
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a, b) => a.Patient_Appointment_No.localeCompare(b.Patient_Appointment_No),
    },
    {
      title: 'Patient Name',
    ...
    },
    {
      ...
      render:(Amount_Recieved)=>{
         const formattedAmount = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'KES' }).format(Amount_Recieved);
        ...
        render:(Document_Date)=>{
            const formattedDate = new Date(Document_Date).toLocaleDateString('en-GB', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
              });
  ...
  return (
    <>
      <h4 style={{ color: "#0f5689" }}>Previous Bill-Cash</h4>
       <div style={{ marginTop: "20px" , flexDirection: "row", alignItems: "center"}}>
        ...
        />
            ...
            onClick={() => {
                const filtered = receiptHeaders.filter(item =>
                item.Patient_No?.toString().toLowerCase().includes(searchText.toLowerCase()) ||
                item.Patient_Appointment_No?.toString().toLowerCase().includes(searchText.toLowerCase()) ||
                item.Received_From?.toString().toLowerCase().includes(searchText.toLowerCase())
                );
                setFilteredData(filtered);
                setIsSearchPerformed(true);
            }}
