...
import { Modal, Form, Input, Select, DatePicker, Button, Row, Col, Checkbox, message } from "antd";
...
const patientNumber= new URLSearchParams(window.location.search).get("PatientNo");
  ...
  const { loading:processReceiptLoading } = useSelector((state) => state.processReceipt);
...
const patientDetails=useFetchPatientDetailsHook(patientNumber );
  ...
  const handleOk = async () => {
    try {
      ...
      try {
        // Post receipt header and get the new receipt number
        ...
        message.error("An error occurred while processing the receipt.");
      }
    } catch (error) {
