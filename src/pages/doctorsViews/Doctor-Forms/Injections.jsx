import { Button, Col, DatePicker, Form, Input, message, Row, Select, TimePicker, Divider, Table, Typography } from 'antd'
import TextArea from 'antd/es/input/TextArea'
import { SaveOutlined,MedicineBoxOutlined } from '@ant-design/icons';
...
const Injections = ({ observationNumber }) => {
    ...
    const onFinish = (values) => {
        const { injectionNo, injectionQuantity, injectionRemarks, itemNo, location, injectionUnitOfMeasure, duration } = values.injections;
    ...
    return (
      ...
      >
                        <MedicineBoxOutlined style={{ marginRight: "8px", fontSize: "20px" }} />  {/* Injection Icon */}
                        Injections
      </Typography.Title>
                    <Form layout="vertical" validateTrigger="onChange" onFinish={onFinish} initialValues={{
                        injections: {
                            observationNumber: '',
                            injectionNo: '',
                            itemNo: '',
                      ...
                      <Row gutter={16}>
                            <Col span={8}>
                                <Form.Item label="Injection Location" name={['injections', 'location']} hasFeedback>
                                    <Select style={{ width: '100%' }} optionFilterProp="label" placeholder="Select Location" name='location'>
                            ...
                            <Col span={8}>
                                <Form.Item label="Injection Items" name={['injections', 'itemNo']} hasFeedback>
                                    <Select style={{ width: '100%' }} optionFilterProp="label" placeholder="Select Items" name='itemNo'>
                                        {items && items.map((item, index) => (
                                            <Select.Option key={index} value={item.No} label={item.No}>
                                                {item.Description}
                                            </Select.Option>
                                        ))}
                                    </Select>
                            ...
                            <Col span={8}>
                            <Form.Item label="Unit of measure" name={['injections', 'injectionUnitOfMeasure']}
            hasFeedback
        >
        <Select 
        ...
        >
                        ...
                        <Row gutter={16}>
                            <Col span={8}>
                                <Form.Item label="Injection Quantity" name={['injections', 'injectionQuantity']} hasFeedback>
                                    <Input type='number' name='injectionQuantity' />
                                </Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item label="Injection No" name={['injections', 'injectionNo']} hasFeedback rules={[{ required: true, message: 'Please input injection no!' }]}>
                                    <Select style={{ width: '100%' }} optionFilterProp="label" placeholder="Select Injection No">
                            ...
                            <Col span={8}>
                                <Form.Item label="Duration " name={['injections', 'duration']} hasFeedback>
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={20}>
                                <Form.Item label="Injection Remarks" name={['injections', 'injectionRemarks']} hasFeedback>
                                    <TextArea autoSize={{ minRows: 3, maxRows: 5 }} name='injectionRemarks' />
                                </Form.Item>
                            </Col>
                        </Row>
                        <Row gutter={16}>
                    ...
                    {getInjections && Object.keys(getInjections).length > 0 && (
                        <div style={{ marginTop: '10px' }}>
                            <Divider />
                            <Table columns={columns} dataSource={dataSource} pagination={false} expandable={{
