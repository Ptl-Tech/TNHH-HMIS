...
const TriageListPending = () => {
  ...
  useEffect(() => {
    const filterData = () => {
      if (
        filterWaitingListType !== "" &&
        searchQueryWaitingList.trim() !== ""
      ) {
        return waitingListTableDataSource.filter((item) => {
          if (filterWaitingListType === "name") {
            return item.name
              .toLowerCase()
              .includes(searchQueryWaitingList.toLowerCase());
          }
  ...
  return (
    <div style={{ padding: "10px 10px" }}>
      ...
      <Card style={{ padding: "24px 10px 10px 10px" }}>
        <TriageFilters
          setFilterWaitingListType={setFilterWaitingListType}
          filterWaitingListType={filterWaitingListType}
          setSearchQueryWaitingList={setSearchQueryWaitingList}
        />
